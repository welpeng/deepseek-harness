...
describe('ui-layout client apply', () => {
  ...
  it('provides layout and Activities and registers AppFrame with its child declarations', async () => {
    const { ctx, slots } = await bench()
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    expect(ctx.get('layout')).toBeInstanceOf(LayoutController)
    expect(ctx.get('activities')).toBeInstanceOf(ActivityRegistry)
    ...
    expect(slots.spec('activity.main')).toEqual({ kind: 'keyed', scope: 'root' })
  })

  it('injects the Activity hook source and attaches the layout actions', async () => {
    const { ctx, slots } = await bench()
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    const actions = {
    ...
    expect(injected).toEqual({
      hooks: { activities: ctx.get('activities') },
      defaultActivityId: 'sessions',
    })
  ...
  it('teardown unwinds the service, the root registration, and the child declarations', async () => {
    ...
    expect(ctx.get('activities')).toBeUndefined()
    expect(slots.entries('root')).toHaveLength(0)
    expect(slots.spec('sidebar')).toBeUndefined()
    expect(slots.spec('activity.main')).toBeUndefined()
