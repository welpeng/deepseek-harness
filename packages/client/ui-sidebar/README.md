...
Sidebar shell plugin: the wordmark, Activity switcher, New Session action, layout-owned collapse control, scroll-aware region seat, and bottom-pinned Settings seat. [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`; this package neither derives its rows nor owns its view preferences. A non-Session Activity contributes its navigation metadata through `ctx.activities` and its own sidebar under the matching key in `sidebar.activity`. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).

The built-in Sessions Activity is the default and retains the existing New Session and Workspace surfaces. The Activity switcher appears only when a second Activity is composed, follows registry order and live locale labels, and dispatches the selected Activity's keyed sidebar without importing its feature package.
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces`, keyed `sidebar.activity`, and `sidebar.settings` child slots, plus injected Activity navigation, `startSession`, and sidebar-toggle callbacks. There is no plugin store.

During a live collapse, the shell holds the expanded content at its current width while it fades out for 150ms. The live upper controls—the shell toggle, optional Activity tabs, the Session-only New Session control, and controls rendered by the selected browsing region—then share one 150ms fade and 49px leftward translation into the 56px rail, ending with the layout's 300ms column slide; every 36px control box follows the same path to the rail's 10px left inset. The bottom-pinned `sidebar.settings` control shares the fade timing but has no horizontal translation. A page that starts collapsed renders the rail statically, and reduced-motion mode disables both transitions.
...
- **Activity chrome carries navigation metadata only** — each Activity plugin must contribute matching main and sidebar entries and own its data lifecycle.
