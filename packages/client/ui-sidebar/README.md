...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared brand, `sidebar.workspaces`, keyed `sidebar.activity`, `sidebar.settings`, and `sidebar.footer.action` child slots, plus injected Activity navigation, `startSession`, and sidebar-toggle callbacks. There is no plugin store.
